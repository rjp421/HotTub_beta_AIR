...
//private var __chatStyleSheet:StyleSheet = new StyleSheet();
...
private function showMsg(htmlText:String):void
{
	...
	if (__chatMessageHistory_Vec.length >= __maxNumLines)
	{
		...
		__chatMessageHistory_Vec.splice(((__chatMessageHistory_Vec.length - __maxNumLines) >= 0) ? (__chatMessageHistory_Vec.length - __maxNumLines) : 0, 1);
	...
	//chatTextArea.chatTextArea_TA.textDisplay.selectRange(int.MAX_VALUE, int.MAX_VALUE);
	//chatTextArea.chatTextArea_TA.invalidateDisplayList();
	//chatTextArea.chatTextArea_TA.validateNow();
