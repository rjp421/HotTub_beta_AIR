...
{
	debugMsg("chatPanelCreationCompleteHandler->  LOADED");
...
public function receiveMessage(event:CustomEvent):void
{
	...
	var msgObj:Object = event.cloneCustomEvent().eventObj;
	var userObj:Object = parentApplication.lobby.userListPanel.getUserObj(msgObj.userID);
	
	if (!parentApplication.lobby.isUserIgnored(userObj.acctID, (userObj.acctID==0 ? userObj.userName : userObj.nonDuplicateName)) && !userObj.hasBlocked)
	{
...
public function sendMessageToServer():void
{
	...
	if (chatTextInput.chatTextInput_TI.text.split(" ")[0].toLowerCase() == "/bw")
	{
		AppWideEventDispatcher_Singleton.getInstance().dispatchEvent(new Event("doBWCheck", false, false));
...
{
	var tmpObj:Object = event.cloneCustomEvent().eventObj;
	debugMsg("showAdminMessage->  msg: "+tmpObj.msg);
